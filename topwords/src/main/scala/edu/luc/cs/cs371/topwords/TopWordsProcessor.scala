...
case class WindowState(
  window: Vector[String],//replacement for mutable Queue, Vector is faster than List : https://www.scala-lang.org/docu/files/collections-api/collections_15.html
...
object TopWordsProcessor:
  ...
  def updateState(state: WindowState, newWord: String, windowSize: Int): WindowState = 
    ...
    val updatedFreqs = state.freqs.updatedWith(newWord):
    ...
    if updatedWindow.size > windowSize then
      ...
      //re-eval freqs
      val trimmedFreqs = updatedFreqs.updatedWith(evictedWord): //either subtract one or remove freq count
  ...
  private def generateWordCloud(state: WindowState, cloudSize: Int): List[(String, Int)] =
    state.freqs
      .toList
